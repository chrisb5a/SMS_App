    ...
    @IBAction func sendPressed(_ sender: UIButton) {
       ...
       // self.ref.child("users").child(user.uid).setValue(["username": username])
        ...
        let postObject = [
                            ...
                            "messageId": idDate,
                            "userSenderId": Auth.auth().currentUser?.email ?? "Unknown",
...
extension ChatViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.label.text = "\(self.post[indexPath.row]["userSenderId"]!)" + ":" + "\n" + "\n" + "\(self.post[indexPath.row]["content"]!)"
        //cell.label.text = posts
        
        if posts["userSenderId"] as? String  == Auth.auth().currentUser?.email {
